...
{
    redis = redisConnect("192.168.100.79", 6379);
...
void redisControl::setpublishForceData(std::vector<double> &data)
{
    ...
    transfromJson(keys, data, out);
}

int redisControl::publish( const string &channel)
{
//    std::string str = "publish XZJQR_DATAARR %s";
    redisReply *reply = (redisReply*)redisCommand(redis,std::move(channel.c_str()), std::move(out.c_str()));    // 执行命令，结果强转成redisReply*类型
    if( NULL == reply)
    {
